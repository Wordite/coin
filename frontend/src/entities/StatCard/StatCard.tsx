...
const StatCard = ({ title, value }: StatCardProps) => (
  <div className='relative w-full min-h-[8.375rem] rounded-xxl p-[1px] bg-gradient-purple-blue-3'>
    <div className='relative w-full h-full bg-gray-transparent-10 rounded-xxl p-[1.188rem] border-1 border-stroke-light hover:scale-105 duration-200 overflow-hidden'>
      ...
      <div className='relative flex flex-col justify-between h-full'>
        <p className='text-[1.063rem] font-bold text-white-transparent-75 max-md:text-[1.2rem]'>{title}</p>
        <p className='text-[1.75rem] leading-none font-semibold mt-[1.25rem] max-md:text-[1.8rem]'>{value}</p>
      </div>
    </div>
  </div>
)
